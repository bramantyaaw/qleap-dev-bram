...
const ChildBorn = (props) => {
  ...
  const [fileFam, setFileFam] = useState(null);
  const [fileUploadedFam, setFileUploadedFam] = useState("");
  const [folderUploadedFam, setFolderUploadedFam] = useState("");
  const [mimeTypeFam, setMimeTypeFam] = useState("");
  const [sizePhotoFam, setSizePhotoFam] = useState(0);
  ...
  const onChangeFamilyPicture = (e) => {
    if (e.target.files[0].size >= 1000000) {
      setWarningFile(true);
    } else {
      setFileFam(e.target.files[0]);
    }
  };
      ...
      await axios
        ...
        .then((res) => {
          if (res?.status === 200) {
            ...
            const newMime = res?.data?.data?.mime_type;
            const newSize = res?.data?.data?.size;
            setFileUploadedFam(newFile);
            setFolderUploadedFam(newFolder);
            setMimeTypeFam(newMime);
            setSizePhotoFam(newSize);
          } else {
            return res;
          }
        });
    ...
    <Form className="first-marriage-wrapper">
      <Card className="mb-3 border-0">
        <Card.Header className="border-bottom px-3 py-3">
          <h6 className="mb-0">
            Detail Tunjangan Sukacita Kelahiran Anak Pertama
          </h6>
        </Card.Header>
        <Card.Body className="submit-data-menu">
          ...
          <Form.Group>
            <TextForm
              text="Submit Foto Surat Keterangan Lahir/Akta Kelahiran"
              className="mt-3 "
            />
            <FileInput
          ...
          <Form.Group>
            <TextForm text="Submit Foto Kartu Keluarga" className="mt-3 " />
            <FileInput
              handleChange={onChangeFamilyPicture}
              accept="image/png, image/jpg, image/jpeg"
            />
