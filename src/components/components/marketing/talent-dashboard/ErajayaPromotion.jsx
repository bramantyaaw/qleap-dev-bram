...
const ErajayaPromotion = ({ arrEmployeePromotion, selectedUid }) => {
  ...
  let countPage = Math.ceil(arrEmployeePromotion?.length / ticketPerPage);
  ...
  useEffect(() => {
    let dataSliced =
      arrEmployeePromotion?.length <= 5
        ? arrEmployeePromotion
        : arrEmployeePromotion?.slice(pageVisited, pageVisited + ticketPerPage);

    setArrDataSliced(dataSliced);
    // eslint-disable-next-line
  }, [pageVisited, ticketPerPage, arrEmployeePromotion]);
    ...
    <>
      {selectedUid !== "" ? (
        arrEmployeePromotion?.length !== 0 ? (
          ...
          <NoArrComponent
            text="There is no work  promotion in Erajaya"
            className="py-10"
          />
        )
      ) : (
...
export default ErajayaPromotion;
